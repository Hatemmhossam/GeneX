# -*- coding: utf-8 -*-
"""TwinSimulation_merged_Clean (2).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Um4DeTSCF_18Ji53gjfKVFvWo-GGJ4bv

# TwinSimulation

This notebook refactors the merged workflow into backend-friendly Python services


> You can still set example file paths below for notebook testing, but the actual pipeline now accepts patient file path, sample selection, and candidate drugs dynamically.
"""
...
PATHWAYS = {
    "Inflammatory_Response": ["IL6", "TNF", "STAT3", "JAK1", "CXCL8", "NFKB1", "NFKBIA"],
    "T_Cell_Activation": ["FOXP3", "CD28", "IL2RA", "CD4", "CD8A", "LCK", "ZAP70"],
    "Antigen_Presentation": ["HLA-B", "HLA-DPB1", "HLA-DQB1", "B2M", "TAP1", "TAP2"],
    "JAK_STAT_Signaling": ["JAK1", "JAK2", "STAT1", "STAT3", "STAT5A", "STAT5B"],
    "Apoptosis_Cell_Stress": ["BCL2", "BAX", "CASP3", "CASP8", "TP53", "MCL1"],
}

"""## Helper utilities

These stay generic so they can later move into plain `.py` service files with minimal changes.
...
"""## Shared preprocessing service

This section removes hardcoded patient dependence from the core logic. The notebook can still use example paths, but every loader now accepts inputs dynamically.
...
This is the cleaned Layer 1 service. It preserves explicit multi-model comparison but removes LightGBM.
...
This is the cleaned Layer 2 service. It exposes twin preparation, training, patient alignment, single-drug simulation, and combination simulation in callable methods.
...
This section merges the ranking and twin layers and returns app-friendly tables.
...
class FusionService:
    ...
    def fuse_single(self, df_rank: pd.DataFrame, twin_single: pd.DataFrame):
        ...
        if twin_single.empty:
            ...
            fused_single["combined_score"] = fused_single["final_score"]
            return fused_single
        ...
        fused_single = df_rank_merge.merge(
            twin_single_merge[["drug_name_norm", "risk_reduction_pct", "n_valid_targets_in_twin", "top_pathway"]],
            on="drug_name_norm",
            how="left",
        ).copy()

        fallback = fused_single["risk_reduction_pct"].min(skipna=True) if fused_single["risk_reduction_pct"].notna().any() else 0.0
        fused_single["score_twin"] = minmax01(fused_single["risk_reduction_pct"].fillna(fallback))
        ...
        fused_single["combined_score"] = (
            ...
            self.config.fusion_w_path * fused_single["pathway_coverage_score"]
        )
...
"""## One pipeline function for notebook testing and future API use

This is the piece you can later call from Django or FastAPI. It returns clean objects instead of relying on notebook prints.
...
"""## App/backend-ready helper methods

These utility functions show the kind of payloads your API layer can return to Flutter later.

"""

def build_drug_detail_payload(drug_name: str, fused_single: pd.DataFrame, trainable: pd.DataFrame, twin_single: pd.DataFrame):
    payload = {"drug_name": drug_name, "found": False}
    dn = norm_basic(drug_name)

    rank_row = fused_single[fused_single["drug_name"].map(norm_basic) == dn]
    train_row = trainable[trainable["prism_match"].map(lambda x: norm_basic(x) if pd.notna(x) else x) == dn]
    twin_row = twin_single[twin_single["drug_name"].map(norm_basic) == dn]

    if rank_row.empty and train_row.empty and twin_row.empty:
        return payload
    ...
    if not rank_row.empty:
        rr = rank_row.iloc[0]
        payload["ranking"] = {
            "best_model_name": rr.get("best_model_name"),
            "final_score": rr.get("final_score"),
            "combined_score": rr.get("combined_score"),
            "pred_ic50": rr.get("pred_ic50"),
            "recommendation": rr.get("recommendation"),
            "score_conf": rr.get("score_conf"),
        }
    if not train_row.empty:
        tr = train_row.iloc[0]
        payload["targets"] = {
            "direct_targets": tr.get("direct_targets", []),
            "indirect_targets": tr.get("indirect_targets", []),
            "n_main_targets": tr.get("n_main_targets"),
            "n_indirect_targets": tr.get("n_indirect_targets"),
        }
    if not twin_row.empty:
        tw = twin_row.iloc[0]
        payload["twin"] = {
            "risk_reduction_pct": tw.get("risk_reduction_pct"),
            "top_pathway": tw.get("top_pathway"),
            "pathway_hits": tw.get("pathway_hits"),
            "gene_changes": tw.get("gene_changes"),
        }
    return payload

"""## Export example

Keep CSV exports for debugging or app ingestion.

"""

def export_pipeline_outputs(results: Dict[str, Any], export_dir="phase1_clean_outputs"):
    os.makedirs(export_dir, exist_ok=True)

    if not results["ranking_bundle"]["df_rank"].empty:
        results["ranking_bundle"]["df_rank"].to_csv(os.path.join(export_dir, "layer1_ranked_single_drugs.csv"), index=False)
    if not results["fused_single"].empty:
        results["fused_single"].to_csv(os.path.join(export_dir, "merged_single_drug_ranking.csv"), index=False)
    if not results["twin_single"].empty:
        results["twin_single"].drop(columns=["gene_changes"], errors="ignore").to_csv(
            os.path.join(export_dir, "digital_twin_single_drugs.csv"), index=False
        )
    if not results["combo_rank"].empty:
        results["combo_rank"].drop(columns=["gene_changes"], errors="ignore").to_csv(
            os.path.join(export_dir, "digital_twin_combinations.csv"), index=False
        )
    return export_dir